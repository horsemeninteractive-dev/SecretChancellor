  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <motion.div 
        ...
        className="bg-[#1a1a1a] border border-[#333] rounded-2xl p-6 max-w-lg w-full shadow-2xl flex flex-col max-h-[80vh]"
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-thematic text-white flex items-center gap-2">
        ...
        <div className="flex gap-4 mb-6 border-b border-[#333]">
          <button 
            ...
            className={cn("pb-2 border-b-2 font-mono text-sm uppercase tracking-widest transition-colors", activeTab === 'ELO' ? "border-yellow-500 text-white" : "border-transparent text-[#666]")}
          >
            ELO
          </button>
          <button 
            onClick={() => setActiveTab('Win%')} 
            className={cn("pb-2 border-b-2 font-mono text-sm uppercase tracking-widest transition-colors", activeTab === 'Win%' ? "border-emerald-500 text-white" : "border-transparent text-[#666]")}
          >
            Win%
          </button>
          <button 
            onClick={() => setActiveTab('Games')} 
            className={cn("pb-2 border-b-2 font-mono text-sm uppercase tracking-widest transition-colors", activeTab === 'Games' ? "border-blue-500 text-white" : "border-transparent text-[#666]")}
          >
            Games
          </button>
        </div>
        ...
        <div className="space-y-2 overflow-y-auto custom-scrollbar flex-1 mb-6">
          {loading ? (
            <div className="text-center text-[#666] py-10">Loading...</div>
          ) : (
            <div className="flex items-center gap-4 px-3 text-[10px] uppercase tracking-widest text-[#666] font-mono mb-2">
          ...
          {sortedData.map((u, i) => {
            ...
            return (
              <div key={u.id} className={cn("flex items-center gap-4 p-3 rounded-xl border", u.id === user.id ? "bg-red-900/10 border-red-900/50" : "bg-[#222] border-[#333]")}>
                <div className="w-8 text-center font-mono text-[#666] shrink-0">{getRankDisplay(i)}</div>
                <div className="flex-1 font-medium text-white truncate" title={u.username}>{displayName}</div>
        ...
        {currentUserData && (
          <div className="pt-4 border-t border-[#333]">
            <div className="text-[10px] uppercase tracking-widest text-[#666] font-mono mb-2">Your Rank</div>
            <div className="flex items-center gap-4 p-3 bg-[#222] rounded-xl border border-red-900/50">
              <div className="w-8 text-center font-mono text-[#666] shrink-0">#{currentUserRank}</div>
              <div className="flex-1 font-medium text-white truncate">{currentUserData.username}</div>
